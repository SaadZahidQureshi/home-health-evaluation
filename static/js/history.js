let customer_enndpoint = `/api/customers/healthy-home`;
let customers_data = []
...
async function deleteCustomer(id){
    try {
        ...
        let response = await requestAPI(`/api/customers/healthy-home/${id}`, null, headers, 'DELETE');
...
async function sendReport(id){
    try {
        ...
        let response = await requestAPI(`/api/principles/report?customer_id=${id}`, null, headers, 'POST');
        if (response.status == 200) {
